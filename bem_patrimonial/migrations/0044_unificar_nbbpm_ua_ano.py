# Generated by Django 5.2.15 on 2026-08-26 17:23 — reescrito para UA (prefixo UA, sequencial por UA/ano)
import re
...
def extrair_codigo_ua(codigo_completo):
    cod = str(codigo_completo or "").strip()
    if not cod:
        return "000"
    apenas = re.sub(r"\D", "", cod)
    if not apenas:
        return "000"
    if "." in cod:
        ultimo = cod.split(".")[-1]
        seg = re.sub(r"\D", "", ultimo)
        if seg:
            num = seg.lstrip("0") or "0"
            return num.zfill(3)[-3:]
        num = apenas.lstrip("0") or "0"
        return num.zfill(3)[-3:]
    num = apenas.lstrip("0") or "0"
    return num.zfill(3)[-3:]
    ...
    try:
        ...
        for baixa in baixas_sem_numero.iterator():
            if NBBPM.objects.using(db_alias).filter(baixas__in=[baixa]).exists():
                continue
            try:
                ua = UnidadeAdministrativa.objects.using(db_alias).get(pk=baixa.unidade_administrativa_origem_id)
            except Exception:
                continue
            if not ua:
                continue
            prefixo = extrair_codigo_ua(getattr(ua, "codigo", ""))
            data_aut = None
            if getattr(baixa, "data_aprovacao", None):
                try:
                    data_aut = baixa.data_aprovacao.date() if hasattr(baixa.data_aprovacao, "date") else baixa.data_aprovacao
            ...
            qs_nbbpm = NBBPM.objects.using(db_alias).filter(numero__startswith=f"{prefixo}.", numero__endswith=f".{ano}").exclude(numero__exact="")
            try:
            ...
            qs_baixa = Baixa.objects.using(db_alias).filter(
                ...
                numero_nbbpm__isnull=False,
            ).exclude(numero_nbbpm__exact="").filter(numero_nbbpm__endswith=f".{ano}")
            try:
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.AlterField(
            model_name='nbbpm',
            name='numero',
            field=models.CharField(blank=True, db_index=True, default='', help_text='Gerado automaticamente, no formato <COD_UA>.<SEQUENCIAL_7>.<ANO>', max_length=32, validators=[django.core.validators.RegexValidator(message='Número NBBPM deve estar no formato XXX.YYYYYYY.ZZZZ', regex='^\\d{3}\\.\\d{7}\\.\\d{4}$')], verbose_name='Número da NBBPM'),
        ),
        migrations.AddIndex(
            model_name='nbbpm',
